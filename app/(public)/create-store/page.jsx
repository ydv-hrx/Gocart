...
export default function CreateStore() {
    ...
    const fetchSellerStatus = async () => {
        ...
        try {
            const { data } = await axios.get('/api/store/status', { headers: { Authorization: `Bearer ${token}` } })
            console.log('Store status from API:', data.status)
            if(['approved', 'rejected', 'pending'].includes(data.status)) {
                ...
                switch (data.status) {
                    ...
                    case "rejected":
                        ...
                        break;
                    case "pending":
                        setMessage("Your store request is still pending, please wait for the admin to review your request")
                        break;
                    default:
                        break;
                }
    ...
    if(!user){
       return (
        <div className="min-h-[80vh] mx-6 flex items-center justify-center text-slate-400">
            <h1 className="text-2xl sm:text-4xll font-semibold">Please <span className="text-slate-500">Login</span> to continue</h1>
        </div>
       ) 
    }
